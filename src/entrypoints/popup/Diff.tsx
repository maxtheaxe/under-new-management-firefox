...
import type { IExtensionDeveloperInformation } from '@/utils/interfaces';
...
const Diff = ({ obj1, obj2 }: DiffProps) => {
	...
	const findDifferences = (
		...
		(Object.keys(obj1) as (keyof IExtensionDeveloperInformation)[]).forEach(
			(key) => {
		...
		(Object.keys(obj2) as (keyof IExtensionDeveloperInformation)[]).forEach(
			(key) => {
				if (!Object.hasOwn(obj1, key)) {
					diff.added[key] = obj2[key];
				} else if (obj1[key] !== obj2[key]) {
					diff.added[key] = obj2[key];
					if (!Object.hasOwn(diff.removed, key)) {
						diff.removed[key] = obj1[key];
					}
				}
			},
				...
				<pre className="text-wrap">
					{Object.keys(differences.removed).map((key) => (
						<div key={key} className="text-red-500">
							{key}:{' '}
							{differences.removed[
								key as keyof IExtensionDeveloperInformation
							] ?? 'null'}
						</div>
					))}
				...
				<pre className="text-wrap">
					{Object.keys(differences.added).map((key) => (
						<div key={key} className="text-green-500">
							{key}:{' '}
							{differences.added[key as keyof IExtensionDeveloperInformation] ??
								'null'}
						</div>
					))}
					{Object.keys(differences.unchanged).map((key) => (
